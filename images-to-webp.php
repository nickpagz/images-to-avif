<?php
/*
	Plugin Name: Images to avif
	Plugin URI: https://www.paypal.me/jakubnovaksl
	Description: Convert JPG, PNG and GIF images to AVIF, speed up your web
	Version: 1.0
	Author: nickpagz
	Author URI: https://nickp.blog
	Text Domain: images-to-avif
	Domain Path: /languages
*/

defined('ABSPATH') || exit;

class images_to_avif{
	var $plugin_admin_page;
	var $settings;
	var $tab;
	var $extensions = array( 'jpg', 'jpeg', 'gif', 'png', 'avif' );

	function __construct(){
		$this->settings = get_site_option('images_to_avif_settings');
		...
		add_action( 'fly_image_created', array( $this, 'fly_images_to_avif' ), 10, 2 );
		add_action( 'bis_image_created', array( $this, 'bis_images_to_avif' ), 10, 2 );
	}

	function admin_enqueue_scripts( $hook ){
		if( $hook == 'media_page_images-to-avif' ){
			if( isset( $_GET['tab'] ) && $_GET['tab'] == 'convert' ){
				...
				wp_add_inline_script( 'jstree', 'var transparency_status_message = "' . __( 'Please wait, converting your images is in progress...', 'images-to-avif' ) . '", error_message = "' . __( 'Error {{ERROR}}, trying to continue with missing images... ', 'images-to-avif' ) . '"' );
	...
	function plugins_loaded(){
		load_plugin_textdomain( 'images-to-avif', FALSE, basename( dirname( __FILE__ ) ) . '/languages/' );
	}
	...
	function activate(){
		...
		// active flag
		update_site_option( 'active_images_to_avif', 1 );
		// maybe load default settings
		if( ! $this->settings = get_site_option( 'images_to_avif_settings', 0 ) ){
			...
			update_site_option( 'images_to_avif_settings', $default_options );
	...
	function plugin_menu_link(){
		$this->plugin_admin_page = add_submenu_page(
			'upload.php',
			__( 'Images to avif', 'images-to-avif' ),
			__( 'Images to avif', 'images-to-avif' ),
			'manage_options',
			basename( __FILE__ ),
	...
	function plugin_admin_tabs( $current = 'general' ){
		$tabs = array(
			'general' => __( 'General', 'images-to-avif' ),
			'convert' => __( 'Convert existing images', 'images-to-avif' ),
		); ?>
	...
	function admin_options_page(){
		...
		if( isset( $_POST['plugin_sent'] ) ){
			if( check_admin_referer('itw_general') ){
				...
				$this->settings['avif_quality'] = intval( $_POST['avif_quality'] );
				if( $this->settings['avif_quality'] < 0 || $this->settings['avif_quality'] > 100 ){
					$this->settings['avif_quality'] = 100;
				}
				...
				update_site_option( 'images_to_avif_settings', $this->settings );
				flush_rewrite_rules( true );
			}
		} ?>
		<div class="wrap">
			<h2><?php _e( 'Images to avif', 'images-to-avif' ); ?></h2>
	...
	function convert_image( $file ){
		if( is_file( $file ) ){
			...
			if( in_array( $image_extension, $this->settings['extensions'] ) ){
				...
				$convert = new avif_converter();
				$response = $convert->convertImage( $file, $this->settings['avif_quality'] );
				if( $response ){
					...
					if( is_dir( $folder ) ){
						...
						if( substr( $folder, 0, $secure_path_len ) === $secure_path ){
							...
							foreach( $files as $file ){
								if( ! $only_missing || ! file_exists( $folder . '/' . $file . '.avif' ) ){
									$converted += $this->convert_image( $folder . '/' . $file ) ? 1 : 0;
								}
							}
							printf( __( '%d converted', 'images-to-avif' ), $converted );
						}
					}
				}
			}
	...
	function fly_images_to_avif( $attachment_id, $fly_file_path ){
		$this->convert_image( $fly_file_path );
	}

	function bis_images_to_avif( $attachment_id, $bis_file_path ){
		$this->convert_image( $bis_file_path );
	}
}

$images_to_avif = new images_to_avif();

register_activation_hook( __FILE__, array( $images_to_avif, 'activate' ) );
register_deactivation_hook( __FILE__, 'deactivate_images_to_avif' );
register_uninstall_hook( __FILE__, 'uninstall_images_to_avif' );

function deactivate_images_to_avif(){
	update_site_option( 'active_images_to_avif', 0 );
	flush_rewrite_rules( true );
}

function uninstall_images_to_avif(){
	delete_site_option('images_to_avif_settings');
	delete_site_option('active_images_to_avif');
}